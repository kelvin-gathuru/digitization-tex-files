...
\modulenumber{9}
\modulename{Root Finding, Numerical Differentiation, and Numerical Integration in Python}
...
\mddescription
This module guides you into a detailed discussion of the concepts of root finding, numerical differentiation and numerical integration using python. Specifically you will learn about use of python in 
\begin{itemize}
\item bisection and newton methods in root finding
...
\begin{mlos}
\item Describe the fundamental concepts and mathematical principles behind root finding methods, numerical differentiation, and numerical integration techniques.
\item Implement Python programs to perform the indicated numerical methods
\item Analyze the accuracy and limitations of numerical differentiation methods when applied to noisy data,\item Design and optimize Python-based numerical algorithms that integrate root finding, numerical differentiation, and numerical integration techniques to solve complex computational problems, including parameter tuning for noise reduction and accuracy improvement.
\end{mlos}
...
Recall that the bisection method uses the intermediate value theorem iteratively to find roots.

\begin{example}
Program a function $my\_bisection(f, a, b, tol)$ that approximates a root $r$ of $f$, bounded by $a$ and $b$ to within $|f(a+b)/2)|<tol$.
\end{example}
...
import numpy as np\\
\\
def my\_bisection(f, a, b, tol): \\
\hspace*{1cm}\# {\it approximates a root, R, of f bounded }\\
\hspace*{1cm}\# {\it by a and b to within tolerance }\\
\hspace*{1cm}\# {\it | f(m) | < tol with m the midpoint }\\
\hspace*{1cm}\# {\it between a and b Recursive implementation}\\
    \\
\hspace*{1cm}\# {\it check if a and b bound a root}\\
\hspace*{1cm}if np.sign(f(a)) == np.sign(f(b)):\\
\hspace*{1.5cm}raise Exception(\\
\hspace*{1.7cm}"The scalars a and b do not bound a root")\\
\\        
\hspace*{1cm}\# {\it get midpoint}\\
\hspace*{1cm}m = (a + b)/2\\
\\    
\hspace*{1cm}if np.abs(f(m)) < tol:\\
\hspace*{1.5cm}\# {\it stopping condition, report m as root}\\
\hspace*{1.5cm}return m\\
\hspace*{1cm}elif np.sign(f(a)) == np.sign(f(m)):\\
\hspace*{1.5cm}\# {\it case where m is an improvement on a. }\\
\hspace*{1.5cm}\# {\it Make recursive call with a = m}\\
\hspace*{1.5cm}return my\_bisection(f, m, b, tol)\\
\hspace*{1cm}elif np.sign(f(b)) == np.sign(f(m)):\\
\hspace*{1.5cm}\# {\it case where m is an improvement on b. }\\
\hspace*{1.5cm}\# {\it Make recursive call with b = m}\\
\hspace*{1.5cm}return my\_bisection(f, a, m, tol)
\end{boxB}
\end{mysolution}

\begin{example}
Approximate $\sqrt{2}$ to a tolerance of $|f(x)|<0.1$ and $|f(x)|<0.01$. Verify that the results are close to a root by plugging the root back into the function.
\end{example}

\begin{mysolution}
The $\sqrt{2}$ can be computed as the root of the function $f(x)=x^2-2$. Starting at $a=0$ and $b=2$, we use my\_bisection to approximate the root.


\begin{boxB}
f = lambda x: x**2 - 2\\
\\
r1 = my\_bisection(f, 0, 2, 0.1)\\
print("r1 =", r1)\\
r01 = my\_bisection(f, 0, 2, 0.01)\\
print("r01 =", r01)\\
\\
print("f(r1) =", f(r1))\\
print("f(r01) =", f(r01))\\
...
\begin{example}
Approximate $\sqrt{2}$ using $x_0=1.$. Compare this approximation with the value computed by Python’s sqrt function.
\[x=1.4-\frac{1.4^2-2}{2(1.4)}=1.4142857142857144\]
\end{example}

\begin{mysolution}
Again, the $\sqrt{2}$ is the root of the function $f(x)=x^2-2$. Using $x_0=1.4$ as a starting point, we have the following code
\begin{boxB}
import numpy as np\\
\\
f = lambda x: x**2 - 2\\
f\_prime = lambda x: 2*x\\
newton = 1.4 - (f(1.4))/(f\_prime(1.4))\\
\\
print("newton =", newton)\\
...
The output is\\
newton $= 1.4142857142857144$\\
sqrt(2) $= 1.4142135623730951$
\end{mysolution}

\begin{example}
\begin{enumerate}[label=(\alph*)]
\item Write a function $my\_newton(f,df,x0,tol)$, where the output is an estimation of the root of $f$, $f$ is a function object $f(x)$, $df$ is a function object to $f'(x)$, $x0$ is an initial guess, and $tol$ is the error tolerance. 
%The error measurement should be |f(x)|
\item Use my\_newton to compute $\sqrt{2}$ to within tolerance of $1e-6$ starting at $x0 = 1.5$.
\end{enumerate}
\end{example}
...
\begin{mysolution}
\begin{enumerate}[label=(\alph*)]
\item Here is the function
\begin{boxB}
def my\_newton(f, df, x0, tol):\\
\hspace*{1cm}\#{\it output is an estimation of the root of f }\\
\hspace*{1cm}\#{\it using the Newton Raphson method}\\
\hspace*{1cm}\#{\it recursive implementation}\\
\hspace*{1cm}if abs(f(x0)) < tol:\\
\hspace*{1.5cm}return x0\\
\hspace*{1cm}else:\\
\hspace*{1.5cm}return my\_newton(f, df, x0 - f(x0)/df(x0), tol)
\end{boxB}
\item Implementation of the function
\begin{boxB}
estimate = my\_newton(f, f\_prime, 1.5, 1e-6)\\
...
\begin{center}
\includegraphics[width=13cm]{EndofMod9Q1sol}
\end{center}
Results from the Python code: $A = 0.3350000000000001$
\end{mysolution}


\item Find the work produced ({\bf W}) in a piston cylinder device using SciPy Module {\bf scipy.integrate}
%\newpage
\begin{mysolution}
\begin{boxB}
